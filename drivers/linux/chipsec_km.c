 ...
 */
long copy_from_kernel_nofault(void * dst,
...
static ssize_t write_mem(struct file * file,
...
static long d_ioctl(struct file * file, unsigned int ioctl_num, unsigned long ioctl_param) {
    ...
    case IOCTL_GET_EFIVAR: {
        ...
        var = (char * ) & kbuf[3];
        stat = myefi -> get_variable(name, & GUID, (u32 * ) & kbuf[2], & var_size,
            var);
